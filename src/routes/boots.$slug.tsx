...
function ProductPage() {
  ...
  const related = products
    .filter((p) => p.slug !== product.slug && p.brand === product.brand && p.limited === product.limited)
    .slice(0, 4);
      ...
      <section className="dark-section pt-24 md:pt-28">
        <div className="mx-auto grid max-w-[1500px] gap-10 px-5 pb-16 md:grid-cols-[1.15fr_1fr] md:gap-16 md:px-10 md:pb-24">
          ...
          <div className="md:pt-6">
            ...
            <h1 className="display mt-5 text-[11vw] text-bone md:text-[3.6rem]">{product.name}</h1>
            {product.subtitle ? (
              <p className="mt-3 text-base text-bone/50">{product.subtitle}</p>
            ) : null}
      ...
      {related.length ? (
        <section className="mx-auto max-w-[1500px] px-5 py-20 md:px-10 md:py-28">
          <Reveal className="flex flex-wrap items-end justify-between gap-4">
            <h2 className="display text-[10vw] md:text-[3.5rem]">More {product.brand}.</h2>
